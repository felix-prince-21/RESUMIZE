...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $experienceId = intval($data->experience_id);
    $job_title = mysqli_real_escape_string($conn, $data->JobTitle);
    ...
    $job_description = mysqli_real_escape_string($conn, $data->JobDescription);

    // Update data in the 'education' table using prepared statements
    $sql = "UPDATE skill SET JobTitle=?, OrganizationName=?, startDate=?, endDate=? , JobDescription = ?, WHERE experience_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $job_title, $organization_name, $start_date, $end_date, $job_description, $experienceId);
